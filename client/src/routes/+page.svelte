<script lang="ts">
  ...
  interface Movie {
    [key: string]: any; // Replace 'any' with the actual type of your movie properties
  }
  ...
  interface FetchMoviesResult {
    ...
    // Event handlers
    const setOpenedPerformance = ({ detail }: { detail: any }) => (openedPerformance = detail as any); // Replace 'any' with the actual type
    const setShowTooltipPopup = ({ detail }: { detail: boolean }) => (showTooltipPopup = detail);
  
    // Fetch movies from the API
    async function fetchMovies(cinemaOids: string[], selectedDate: string, sortBy: string): Promise<FetchMoviesResult | Movie[]> {
    ...
    function createUrlParams(cinemaOids: string[], selectedDate: string, sortBy: string): URLSearchParams {
    ...
    function toggleBodyOverflow(openedPerformance: any | null) { // Replace 'any' with the actual type
    ...
    function getValuesFromLocalStorage(): { cinemaOids?: string[], selectedDate?: string, sortBy?: string } {
  ...
  function getValuesFromUrl(): { cinemaOids?: string[], selectedDate?: string, sortBy?: string } {
    let values: { cinemaOids?: string[]; selectedDate?: string; sortBy?: string } = {};
    ...
    function updateLocalStorage(cinemaOids: string[], selectedDate: string, sortBy: string) {
